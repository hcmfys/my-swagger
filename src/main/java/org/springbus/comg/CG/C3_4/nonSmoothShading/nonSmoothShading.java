package org.springbus.comg.CG.C3_4.nonSmoothShading;
...
// nonSmoothShading.java (スムーズシェーディングしないアプレット）
//	図３−１９（左）

import org.springbus.comg.CG.C3_2.sphdraw.sphdraw;
...
public class nonSmoothShading extends JApplet {
  ...
  public void init() {
    ...
    // スムーズシェーディングしない三角形インデックスフェイスセットの定義
    int numFace = 32;
    int numVertex = 25;
    int i, k;
    ...
    TriangleSet ts1 = new TriangleSet(numFace, ind, numVertex, vtx);
  ...
  public void paint(Graphics g) {
    if (m != null) {
      ...
      if (finished) { // レートレーシングが終了したら画像を描画
        m.drawImage(rayOutImage, -screenx / 2, screeny / 2, screenx, screeny, this);
      } else {
