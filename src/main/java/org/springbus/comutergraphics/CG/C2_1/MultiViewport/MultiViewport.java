package org.springbus.comutergraphics.CG.C2_1.MultiViewport;
...
public class MultiViewport extends JApplet {

  MyCanvas m; // MyCanvasクラスのオブジェクト
  static final int idealSizeX = 600; // ウィンドウの横幅
  static final int idealSizeY = 350; // ウィンドウの縦幅
  ...
  public void paintPIGraph1(MyCanvas m) { // 円グラフ１の描画用のメソッド
    ...
    Font f0 = m.MyFont(m.getFont().getName(), m.getFont().getStyle(), 1.0); // 標準サイズのフォントを生成
    ...
    Font f2 = m.MyFont(m.getFont().getName(), m.getFont().getStyle(), 0.75); // 小さめのフォントを生成
    m.setFont(f2); // フォントの設定
    m.drawString("3", 0.23, -0.04); // 3という文字列を描く（立法メートル用）

    // ビューポート定義　///////////////////////////////////////
    m.setViewport(0.5, 1.0, 0.0, 0.143); // ビューポート２の定義
    Font f = m.MyFont(m.getFont().getName(), m.getFont().getStyle(), 2.0); // 2倍の大きさのフォント生成
    m.setFont(f); // フォントの設定
  ...
  public void paintPIGraph2(MyCanvas m) { // 円グラフ２の描画用のメソッド
    ...
    Font f0 = m.MyFont(m.getFont().getName(), m.getFont().getStyle(), 1.0); // 標準サイズのフォント生成
    ...
    Font f2 = m.MyFont(m.getFont().getName(), m.getFont().getStyle(), 0.75); // ちょっと小さいフォント生成
    m.setFont(f2); // フォントの設定
    m.drawString("3", 0.23, -0.04); // 3という文字列を描く（立法メートル用）
  ...
  public void paint(Graphics g) {
	...
	public static void main(String[] args) {
		MultiViewport m = new MultiViewport();
		m.display();
	}
	}
